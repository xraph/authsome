...
`)
		...
		&migrate.Migration{
			...
			Up: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `ALTER TABLE authsome_app_client_configs ADD COLUMN IF NOT EXISTS mfa_required BOOLEAN;`)
				return err
			},
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `ALTER TABLE authsome_app_client_configs DROP COLUMN IF EXISTS mfa_required;`)
				return err
			},
		},
		// Migration 19: Add family_id column to sessions for refresh-token
		// replay detection (groups sessions descended from a single sign-in).
		&migrate.Migration{
...
CREATE INDEX IF NOT EXISTS idx_authsome_sessions_family_id
    ON authsome_sessions (family_id);
`)
				return err
			},
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `
DROP INDEX IF EXISTS idx_authsome_sessions_family_id;
ALTER TABLE authsome_sessions DROP COLUMN IF EXISTS family_id;
`)
				return err
			},
		},
		// Migration 20: Create authsome_revoked_refresh_tokens table for
...
CREATE INDEX IF NOT EXISTS idx_authsome_revoked_refresh_tokens_family_id
    ON authsome_revoked_refresh_tokens (family_id);
`)
				return err
			},
			Down: func(ctx context.Context, exec migrate.Executor) error {
		...
		// Migration 21: User emails table (multiple emails per account).
...
CREATE TABLE IF NOT EXISTS authsome_user_emails (
    ...
    user_id    TEXT NOT NULL REFERENCES authsome_users(id),
    app_id     TEXT NOT NULL REFERENCES authsome_apps(id),
    env_id     TEXT NOT NULL REFERENCES authsome_environments(id),
    email      TEXT NOT NULL,
    verified   BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_authsome_user_emails_primary
    ON authsome_user_emails (user_id) WHERE is_primary = TRUE AND deleted_at IS NULL;
...
`); err != nil {
				...
				for _, sd := range seeds {
					if _, err := exec.Exec(ctx,
						`INSERT INTO authsome_user_emails (id, user_id, app_id, env_id, email, verified, is_primary, source)
						 VALUES ($1, $2, $3, $4, LOWER($5), $6, TRUE, 'backfill')`,
		...
		// Migration: email-verification OTP support.
		// Adds an attempts counter and replaces the UNIQUE token index with a
		// non-unique one (6-digit OTP codes are not globally unique across
		// users), plus a per-user lookup index for the active-code query.
		&migrate.Migration{
			Name:    "verification_otp_support",
			Version: "20260605000001",
			Up: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `
ALTER TABLE authsome_verifications
...
`)
			...
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `
DROP INDEX IF EXISTS idx_authsome_verifications_active;
