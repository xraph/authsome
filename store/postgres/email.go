...
func (s *Store) CreateUserWithPrimaryEmail(ctx context.Context, u *user.User, primary *user.UserEmail) error {
	...
	if err != nil {
		return fmt.Errorf("authsome/postgres: begin tx for user+email: %w", err)
	}
	defer func() { _ = ptx.Rollback() }() //nolint:errcheck // best-effort rollback after commit

	if _, err := ptx.NewInsert(fromUser(u)).Exec(ctx); err != nil {
		return pgError(err)
	}
	pm := fromUserEmail(primary)
	pm.IsPrimary = true
	if _, err := ptx.NewInsert(pm).Exec(ctx); err != nil {
		return pgError(err)
	}
	if err := ptx.Commit(); err != nil {
		return fmt.Errorf("authsome/postgres: commit user+email: %w", err)
	}
	return nil
}

func (s *Store) AddUserEmail(ctx context.Context, e *user.UserEmail) error {
	_, err := s.pg.NewInsert(fromUserEmail(e)).Exec(ctx)
	return pgError(err)
}
...
func (s *Store) MarkUserEmailVerified(ctx context.Context, userID id.UserID, email string) error {
	...
	ptx, err := s.pg.BeginTxQuery(ctx, nil)
	if err != nil {
		return fmt.Errorf("authsome/postgres: begin tx mark verified: %w", err)
	}
	defer func() { _ = ptx.Rollback() }() //nolint:errcheck // best-effort rollback after commit

	now := time.Now()
	if _, err := ptx.NewUpdate((*UserEmailModel)(nil)).
		Set("verified = ?", true).
		Set("updated_at = ?", now).
	...
	if m.IsPrimary {
		if _, err := ptx.NewUpdate((*UserModel)(nil)).
	...
	if err := ptx.Commit(); err != nil {
		return fmt.Errorf("authsome/postgres: commit mark verified: %w", err)
	}
	return nil
}
...
func (s *Store) SetPrimaryEmail(ctx context.Context, userID id.UserID, email string) error {
	...
	ptx, err := s.pg.BeginTxQuery(ctx, nil)
	if err != nil {
		return fmt.Errorf("authsome/postgres: begin tx set primary: %w", err)
	}
	defer func() { _ = ptx.Rollback() }() //nolint:errcheck // best-effort rollback after commit
	...
	if err := ptx.Commit(); err != nil {
		return fmt.Errorf("authsome/postgres: commit set primary: %w", err)
	}
	return nil
}
...
func (s *Store) DeleteUserEmail(ctx context.Context, userID id.UserID, email string) error {
	...
	now := time.Now()
	_, err = s.pg.NewUpdate((*UserEmailModel)(nil)).
		Set("deleted_at = ?", now).
		Set("updated_at = ?", now).
