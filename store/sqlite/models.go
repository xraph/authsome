...
func fromSession(s *session.Session) *SessionModel {
	...
	// Always encoded, never left nil, for the same reason Roles and Scopes
	// are: the actors column on this backend is NOT NULL DEFAULT '' (unlike
	// postgres's nullable JSONB), and json.RawMessage cannot scan a NULL back.
	// nil marshals to "null", which the decode guard above reads as no actors.
...
type SettingModel struct {
	grove.BaseModel `grove:"table:authsome_settings,alias:st"`

	ID        string          `grove:"id,pk"`
	Key       string          `grove:"key,notnull"`
	Value     json.RawMessage `grove:"value"`
	Scope     string          `grove:"scope,notnull"`
	ScopeID   string          `grove:"scope_id,notnull"`
	AppID     string          `grove:"app_id"`
	OrgID     string          `grove:"org_id"`
	Enforced  bool            `grove:"enforced"`
	Namespace string          `grove:"namespace"`
	Version   int64           `grove:"version"`
	UpdatedBy string          `grove:"updated_by"`
	CreatedAt time.Time       `grove:"created_at,notnull"`
	UpdatedAt time.Time       `grove:"updated_at,notnull"`
}
...
type AppClientConfigModel struct {
	...
	ID                       string          `grove:"id,pk"`
	AppID                    string          `grove:"app_id,notnull"`
	SignupEnabled            *bool           `grove:"signup_enabled"`
	...
	SocialProviders          json.RawMessage `grove:"social_providers"`
	MFAMethods               json.RawMessage `grove:"mfa_methods"`
...
type ServiceAccountModel struct {
	...
	OrgID string `grove:"org_id"`
	// Kind is empty on no row this store writes, but stays nullable-tolerant
	// so a row inserted by hand without it still reads as a service account.
...
func fromServiceAccount(svc *serviceaccount.ServiceAccount) *ServiceAccountModel {
	...
	// ToPrincipal tolerates on read. Every row sqlite inserts or updates gets
	// a concrete kind; the empty-Kind fallback stays a read-time-only
	...
	// Always encoded, never left nil: the scopes column is NOT NULL and
	// json.RawMessage cannot scan a NULL back. nil marshals to "null", which
	// the decode guard on the read side reads as no scopes.
	m.Scopes, _ = json.Marshal(svc.Scopes) //nolint:errcheck // best-effort encode
	if svc.ExpiresAt != nil {
		m.ExpiresAt = sql.NullTime{Time: *svc.ExpiresAt, Valid: true}
	}
	return m
}
...
func fromDelegation(d *principal.Delegation) *DelegationModel {
	...
	// Always encoded, never left nil, for the same reason ServiceAccountModel's
	// Scopes is: the column is NOT NULL and json.RawMessage cannot scan a NULL
	// back.
	m.Scopes, _ = json.Marshal(d.Scopes) //nolint:errcheck // best-effort encode
	if d.ExpiresAt != nil {
		m.ExpiresAt = sql.NullTime{Time: *d.ExpiresAt, Valid: true}
	}
	if d.RevokedAt != nil {
		m.RevokedAt = sql.NullTime{Time: *d.RevokedAt, Valid: true}
	}
	return m
}
