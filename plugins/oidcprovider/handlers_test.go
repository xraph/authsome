...
func setupTestAppOIDCP(t *testing.T) (*bun.DB, *http.ServeMux, string) {
    ...
    if _, err := db.NewCreateTable().Model((*schema.OAuthClient)(nil)).IfNotExists().Exec(ctx); err != nil { t.Fatalf("create oauth_clients: %v", err) }
    ...
    if _, err := db.NewInsert().Model(testUser).Exec(ctx); err != nil { t.Fatalf("create test user: %v", err) }
    ...
    if _, err := db.NewInsert().Model(testSession).Exec(ctx); err != nil { t.Fatalf("create test session: %v", err) }
...
func TestOIDCProvider_Flow(t *testing.T) {
    ...
    if err != nil { t.Fatalf("create authorize request: %v", err) }
    req.AddCookie(&http.Cookie{Name: "session_token", Value: sessionToken})
    resp2, err := client.Do(req)
    if err != nil { t.Fatalf("authorize error: %v", err) }
    if resp2.StatusCode != 302 { t.Fatalf("expected 302 authorize redirect, got %d", resp2.StatusCode) }
    
    // Extract code from redirect URL
    location := resp2.Header.Get("Location")
    if location == "" { t.Fatalf("expected Location header") }
    redirectURL, err := url.Parse(location)
    if err != nil { t.Fatalf("parse redirect URL: %v", err) }
    code := redirectURL.Query().Get("code")
    if code == "" { t.Fatalf("expected code in redirect URL") }
    ...
    if err != nil { t.Fatalf("userinfo error: %v", err) }
    if resp4.StatusCode != 200 { t.Fatalf("expected 200 userinfo, got %d", resp4.StatusCode) }

    // JWKS
    resp5, err := http.Get(srv.URL+"/oauth2/jwks")
    if err != nil { t.Fatalf("jwks error: %v", err) }
    if resp5.StatusCode != 200 { t.Fatalf("expected 200 jwks, got %d", resp5.StatusCode) }
}