...
type CertificateAuthEvent struct {
	...
	// Validation Details
	ValidationSteps map[string]any `bun:"validation_steps,type:jsonb" json:"validationSteps,omitempty"`
...
func (s *StringArray) Scan(value any) error {
	...
	b, ok := value.([]byte)
	if !ok {
		return errs.InternalServerErrorWithMessage("type assertion to []byte failed")
	}
