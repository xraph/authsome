...
func (c *Config) Validate() error {
	...
	if c.AuthMethod != "bearer" && c.AuthMethod != "oauth2" {
		return fmt.Errorf("invalid auth_method: must be 'bearer' or 'oauth2'")
	}
	...
	if c.RateLimit.Enabled {
		if c.RateLimit.RequestsPerMin <= 0 {
			return fmt.Errorf("rate_limit.requests_per_min must be positive")
		}
	...
	if c.BulkOperations.Enabled {
		if c.BulkOperations.MaxOperations <= 0 || c.BulkOperations.MaxOperations > 1000 {
			return fmt.Errorf("bulk_operations.max_operations must be between 1 and 1000")
		}
		if c.BulkOperations.MaxPayloadBytes <= 0 {
			return fmt.Errorf("bulk_operations.max_payload_bytes must be positive")
		}
	}

	if c.Search.MaxResults <= 0 || c.Search.MaxResults > 10000 {
		return fmt.Errorf("search.max_results must be between 1 and 10000")
	}

	if c.Search.DefaultResults <= 0 || c.Search.DefaultResults > c.Search.MaxResults {
		return fmt.Errorf("search.default_results must be between 1 and max_results")
	}

	if c.Webhooks.Enabled {
		if len(c.Webhooks.WebhookURLs) == 0 {
			return fmt.Errorf("webhooks.webhook_urls cannot be empty when webhooks are enabled")
		}
		if c.Webhooks.RetryAttempts < 0 || c.Webhooks.RetryAttempts > 10 {
			return fmt.Errorf("webhooks.retry_attempts must be between 0 and 10")
		}
		if c.Webhooks.TimeoutSeconds <= 0 || c.Webhooks.TimeoutSeconds > 300 {
			return fmt.Errorf("webhooks.timeout_seconds must be between 1 and 300")
		}
	}
