...
func (p *linkedinProvider) FetchUser(ctx context.Context, token *oauth2.Token) (*ProviderUser, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.linkedin.com/v2/userinfo", http.NoBody)
	if err != nil {
		return nil, fmt.Errorf("social: linkedin: create request: %w", err)
	}
