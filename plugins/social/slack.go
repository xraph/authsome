...
func (p *slackProvider) FetchUser(ctx context.Context, token *oauth2.Token) (*ProviderUser, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://slack.com/api/openid.connect.userInfo", http.NoBody)
	if err != nil {
		return nil, fmt.Errorf("social: slack: create request: %w", err)
	}
