...
func postgresSetup(t *testing.T) socialtest.Factory {
	t.Helper()
	...
	core := pgstore.New(db)
	require.NoError(t, core.Migrate(ctx, social.PostgresMigrations), "run migrations")

	plugin := social.NewPostgresStore(db)
	return func(t *testing.T) socialtest.Fixture {
		t.Helper()
...
func mongoSetup(t *testing.T) socialtest.Factory {
	t.Helper()
	...
	plugin := social.NewMongoStore(db)
	return func(t *testing.T) socialtest.Fixture {
		t.Helper()
		return seedFixture(t, core, plugin)
	}
