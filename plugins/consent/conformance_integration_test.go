...
func postgresSetup(t *testing.T) consenttest.Factory {
	t.Helper()
	...
	core := pgstore.New(db)
	require.NoError(t, core.Migrate(ctx, consent.PostgresMigrations), "run migrations")

	plugin := consent.NewPostgresStore(db)
	return func(t *testing.T) consenttest.Fixture {
		t.Helper()
...
func mongoSetup(t *testing.T) consenttest.Factory {
	t.Helper()
	...
	plugin := consent.NewMongoStore(db)
	return func(t *testing.T) consenttest.Fixture {
		t.Helper()
		return seedFixture(t, core, plugin)
	}
