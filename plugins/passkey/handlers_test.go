...
func setupTestAppPK(t *testing.T) (*bun.DB, *http.ServeMux) {
    ...
    if _, err := db.NewCreateTable().Model((*schema.Passkey)(nil)).IfNotExists().Exec(ctx); err != nil { t.Fatalf("create passkeys: %v", err) }
...
func TestPasskey_RegisterFlowPersistsPasskey(t *testing.T) {
    ...
    if err != nil { t.Fatalf("begin request error: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("expected 200, got %d", resp.StatusCode) }
    var beginOut map[string]any
    if err := json.NewDecoder(resp.Body).Decode(&beginOut); err != nil { t.Fatalf("decode begin: %v", err) }
    if _, ok := beginOut["challenge"]; !ok { t.Fatalf("expected challenge in begin response") }
    if beginOut["userId"] != u.ID.String() { t.Fatalf("expected userId %s, got %v", u.ID.String(), beginOut["userId"]) }
    ...
    if err != nil { t.Fatalf("finish request error: %v", err) }
    if resp2.StatusCode != 200 { t.Fatalf("expected 200 finish, got %d", resp2.StatusCode) }
    ...
    if resp3.StatusCode != 200 { t.Fatalf("expected 200 list, got %d", resp3.StatusCode) }
    var listOut []map[string]any
    if err := json.NewDecoder(resp3.Body).Decode(&listOut); err != nil { t.Fatalf("decode list: %v", err) }
    if len(listOut) != 1 { t.Fatalf("expected 1 passkey, got %d", len(listOut)) }
    if listOut[0]["credentialID"] != credID { t.Fatalf("expected credentialID=%s, got %v", credID, listOut[0]["credentialID"]) }
}
...
func TestPasskey_LoginReturnsSession(t *testing.T) {
    ...
    if err != nil { t.Fatalf("begin login request error: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("expected 200, got %d", resp.StatusCode) }
    ...
    if err != nil { t.Fatalf("finish login request error: %v", err) }
    if resp2.StatusCode != 200 { t.Fatalf("expected 200 finish, got %d", resp2.StatusCode) }
    var out map[string]any
    if err := json.NewDecoder(resp2.Body).Decode(&out); err != nil { t.Fatalf("decode finish: %v", err) }
    if _, ok := out["token"].(string); !ok { t.Fatalf("expected token in finish response") }
    if _, ok := out["session"].(map[string]any); !ok { t.Fatalf("expected session in finish response") }
...
func TestPasskey_DeleteRemovesCredential(t *testing.T) {
    ...
    if err != nil { t.Fatalf("finish request error: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("expected 200 finish, got %d", resp.StatusCode) }

    // List to get ID
    resp2, err := http.Get(srv.URL+"/api/auth/passkey/list?user_id="+u.ID.String())
    if err != nil { t.Fatalf("list request error: %v", err) }
    var listOut []map[string]any
    if err := json.NewDecoder(resp2.Body).Decode(&listOut); err != nil { t.Fatalf("decode list: %v", err) }
    if len(listOut) != 1 { t.Fatalf("expected 1 passkey, got %d", len(listOut)) }
    idStr, ok := listOut[0]["id"].(string)
    if !ok || idStr == "" { t.Fatalf("expected id string in list output") }
    ...
    if resp3.StatusCode != 200 { t.Fatalf("expected 200 delete, got %d", resp3.StatusCode) }

    resp4, err := http.Get(srv.URL+"/api/auth/passkey/list?user_id="+u.ID.String())
    if err != nil { t.Fatalf("list request error: %v", err) }
    var listOut2 []map[string]any
    if err := json.NewDecoder(resp4.Body).Decode(&listOut2); err != nil { t.Fatalf("decode list2: %v", err) }
    if len(listOut2) != 0 { t.Fatalf("expected 0 passkeys after delete, got %d", len(listOut2)) }
}