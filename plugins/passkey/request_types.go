...
type FinishRegisterRequest struct {
	UserID   string         `json:"userId"         validate:"required,xid"`
	Name     string         `json:"name,omitempty" validate:"omitempty,min=1,max=100"`
	Response map[string]any `json:"response"       validate:"required"` // WebAuthn PublicKeyCredential
}
...
type FinishLoginRequest struct {
	Response map[string]any `json:"response" validate:"required"` // WebAuthn PublicKeyCredential assertion
