...
func postgresSetup(t *testing.T) passkeytest.Factory {
	t.Helper()
	...
	core := pgstore.New(db)
	require.NoError(t, core.Migrate(ctx, passkey.PostgresMigrations), "run migrations")

	plugin := passkey.NewPostgresStore(db)
	return func(t *testing.T) passkeytest.Fixture {
		t.Helper()
...
func mongoSetup(t *testing.T) passkeytest.Factory {
	t.Helper()
	...
	plugin := passkey.NewMongoStore(db)
	return func(t *testing.T) passkeytest.Fixture {
		t.Helper()
		return seedFixture(t, core, plugin)
	}
