// Package contract wires the impossibletravel plugin's deep-link settings page
...
func Register(
	...
	if deps.Engine == nil {
		return fmt.Errorf("impossibletravel/contract: Engine is required")
	}
	m, err := loader.Load(bytes.NewReader(manifestYAML), "impossibletravel/contract/manifest.yaml")
	if err != nil {
		return fmt.Errorf("impossibletravel/contract: load manifest: %w", err)
	}
	if err := loader.Validate(m, wreg); err != nil {
		return fmt.Errorf("impossibletravel/contract: validate manifest: %w", err)
	}
	if err := reg.Register(m); err != nil {
		return fmt.Errorf("impossibletravel/contract: register manifest: %w", err)
	}
	_ = d
	return nil
}
