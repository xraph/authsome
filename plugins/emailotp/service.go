...
func NewService(r *repo.EmailOTPRepository, users *user.Service, authSvc *auth.Service, auditSvc *audit.Service, provider EmailProvider, cfg Config) *Service {
    // defaults
    if cfg.OTPLength == 0 { cfg.OTPLength = 6 }
...
func (s *Service) SendOTP(ctx context.Context, email, ip, ua string) (string, error) {
    ...
    max := int64(1)
    for i := 0; i < s.config.OTPLength; i++ { max *= 10 }
    code := int64(rand.Intn(int(max)))
    otp := fmt.Sprintf("%0*d", s.config.OTPLength, code)
    // Persist OTP
    if err := s.repo.Create(ctx, e, otp, time.Now().Add(s.config.ExpiresIn)); err != nil { return "", err }
    // Send via provider if available
    if s.provider != nil { _ = s.provider.SendOTP(e, otp) }
    // Audit: email OTP sent
    if s.audit != nil {
        _ = s.audit.Log(ctx, nil, "emailotp_sent", "email:"+e, ip, ua, "")
    }
    // Return OTP only if dev mode
    if s.config.DevExposeOTP { return otp, nil }
    return "", nil
}
...
func (s *Service) VerifyOTP(ctx context.Context, email, otp string, remember bool, ip, ua string) (*auth.AuthResponse, error) {
    ...
    if e == "" || o == "" { return nil, fmt.Errorf("missing fields") }
    rec, err := s.repo.FindByEmail(ctx, e, time.Now())
    if err != nil { return nil, err }
    if rec == nil { return nil, errors.New("otp not found or expired") }
