...
func TestEmailOTP_SendReturnsDevOTP(t *testing.T) {
	...
	if _, ok := out["dev_otp"]; !ok {
		t.Fatalf("expected dev_otp in response for dev mode")
	}
	if out["status"] != "sent" {
...
func TestEmailOTP_VerifyCreatesSession(t *testing.T) {
	...
	if !ok || otpVal == "" {
		t.Fatalf("expected dev_otp string")
	}
...
func TestEmailOTP_VerifyRequiresExistingUserWhenImplicitDisabled(t *testing.T) {
	...
	if _, err := db.NewCreateTable().Model((*schema.EmailOTP)(nil)).IfNotExists().Exec(ctx); err != nil {
		t.Fatalf("create email_otps: %v", err)
	}
...
func TestEmailOTP_SendRespectsRateLimit(t *testing.T) {
	...
	for i := 1; i <= 6; i++ {
		...
		resp, err := http.Post(srv.URL+"/api/auth/email-otp/send", "application/json", bytes.NewReader(buf))
		if err != nil {
			t.Fatalf("send request error #%d: %v", i, err)
		}
		if i <= 5 {
			if resp.StatusCode != 200 {
			...
			if resp.StatusCode != 429 {
				t.Fatalf("expected 429 on attempt %d, got %d", i, resp.StatusCode)
			}
		}
	}
}
