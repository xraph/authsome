...
func NewService(users *user.Service, authSvc *auth.Service) *Service { return &Service{users: users, auth: authSvc} }
...
func (s *Service) SignInWithUsername(ctx context.Context, username, password string) (*auth.AuthResponse, error) {
    ...
    if un == "" || password == "" { return nil, fmt.Errorf("missing fields") }
    u, err := s.users.FindByUsername(ctx, un)
    if err != nil || u == nil { return nil, fmt.Errorf("invalid credentials") }
