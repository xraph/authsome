...
func (h *Handler) Send(c *forge.Context) error {
    ...
    if h.rl != nil {
        ...
        if err != nil { return c.JSON(http.StatusInternalServerError, map[string]string{"error": "rate limit error"}) }
        if !ok { return c.JSON(http.StatusTooManyRequests, map[string]string{"error": "too many requests"}) }
    }
