...
func TestResourceParams(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			req: func(t *testing.T) *http.Request {
				return httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/authorize?client_id=abc", nil)
			},
			want: nil,
		},
		{
			...
			req: func(t *testing.T) *http.Request {
				...
				r := httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/token", strings.NewReader(body))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, resourceParams(tt.req(t)))
		})
	}
}
