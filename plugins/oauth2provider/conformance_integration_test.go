...
func postgresSetup(t *testing.T) oauth2test.Factory {
	t.Helper()
	...
	core := pgstore.New(db)
	require.NoError(t, core.Migrate(ctx, oauth2provider.PostgresMigrations), "run migrations")

	plugin := oauth2provider.NewPostgresStore(db)
	return func(t *testing.T) oauth2test.Fixture {
		t.Helper()
...
func mongoSetup(t *testing.T) oauth2test.Factory {
	t.Helper()
	...
	plugin := oauth2provider.NewMongoStore(db)
	return func(t *testing.T) oauth2test.Fixture {
		t.Helper()
		return seedFixture(t, core, plugin)
	}
