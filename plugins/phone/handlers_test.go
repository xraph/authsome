...
func setupTestAppPhone(t *testing.T) (*bun.DB, *http.ServeMux) {
    ...
    if _, err := db.NewCreateTable().Model((*schema.PhoneVerification)(nil)).IfNotExists().Exec(ctx); err != nil { t.Fatalf("create phone_verifications: %v", err) }
...
func TestPhone_SendReturnsDevCode(t *testing.T) {
    ...
    if _, ok := out["dev_code"].(string); !ok { t.Fatalf("expected dev_code in response for dev mode") }
...
func TestPhone_VerifyCreatesSession(t *testing.T) {
    ...
    if !ok || codeVal == "" { t.Fatalf("expected dev_code string") }
