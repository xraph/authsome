...
func (s *Service) SendCode(ctx context.Context, phone, ip, ua string) (string, error) {
	...
	if err := s.repo.Create(ctx, p, otp, time.Now().Add(s.config.ExpiresIn)); err != nil {
		return "", err
	}
	if s.provider != nil {
		_ = s.provider.SendSMS(p, fmt.Sprintf("Your code: %s", otp))
	}
	if s.audit != nil {
...
func (s *Service) Verify(ctx context.Context, phone, code, email string, remember bool, ip, ua string) (*auth.AuthResponse, error) {
	...
	if rec.Code != c {
		...
		if s.audit != nil {
			_ = s.audit.Log(ctx, nil, "phone_verify_failed", "phone:"+p, ip, ua, "")
		}
		return nil, nil
	}
