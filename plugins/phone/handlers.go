...
func (h *Handler) SendCode(c *forge.Context) error {
    ...
    if h.rl != nil {
        ...
        if err != nil { return c.JSON(500, map[string]string{"error": "rate limit error"}) }
        if !ok { return c.JSON(429, map[string]string{"error": "too many requests"}) }
    }
...
func (h *Handler) Verify(c *forge.Context) error {
    ...
    if err != nil { return c.JSON(400, map[string]string{"error": err.Error()}) }
    if res == nil { return c.JSON(401, map[string]string{"error": "invalid code"}) }
