...
type JSONQuery struct {
	...
	// Populate relations
	Populate any `json:"populate,omitempty"`
...
func (p *JSONParser) parseFilters(filters any) (*FilterGroup, error) {
...
func (p *JSONParser) parseFilterObject(obj map[string]any) (*FilterGroup, error) {
	group := &FilterGroup{
		Operator: LogicalAnd,
	}
	...
	for key, value := range obj {
		...
		case "$not", "not", "NOT":
			subObj, ok := value.(map[string]any)
			if !ok {
...
func (p *JSONParser) parseFilterArray(arr []any, operator LogicalOperator) (*FilterGroup, error) {
	group := &FilterGroup{
		Operator: operator,
	}
...
func (p *JSONParser) parseFieldFilter(field string, value any) ([]FilterCondition, error) {
...
func (p *JSONParser) parseSort(sort any) []SortField {
	var sorts []SortField
...
func (p *JSONParser) parsePopulate(populate any) []PopulateOption {
...
func (p *JSONParser) parsePopulateObject(obj map[string]any) PopulateOption {
	opt := PopulateOption{}
