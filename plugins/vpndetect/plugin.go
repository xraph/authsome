...
// OnBeforePrincipalAuth checks the connection type of a machine caller.
//
// The same check sign-in gets. API-key traffic reaches strategy.Authenticate
// and fires none of the sign-in hooks, so without this a leaked key worked
// from behind a VPN, proxy or Tor exit node regardless of policy.
