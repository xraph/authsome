...
func postgresSetup(t *testing.T) ssftest.Factory {
	t.Helper()
	...
	core := pgstore.New(db)
	require.NoError(t, core.Migrate(ctx, ssf.PostgresMigrations), "run migrations")

	plugin := ssf.NewPostgresStore(db)
	return func(t *testing.T) ssftest.Fixture {
		t.Helper()
...
func mongoSetup(t *testing.T) ssftest.Factory {
	t.Helper()
	...
	require.NoError(t, core.Migrate(ctx, ssf.MongoMigrations), "run migrations")

	plugin := ssf.NewMongoStore(db)
	return func(t *testing.T) ssftest.Fixture {
		t.Helper()
		return seedFixture(t, core, plugin)
	}
