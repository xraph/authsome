...
func (s *Service) NewServiceProvider(entityID, acsURL, metadataURL string) error {
    ...
    // Parse URLs
    if metadataURL != "" {
        if u, err := url.Parse(metadataURL); err == nil { sp.MetadataURL = *u }
    }
    if acsURL != "" {
        if u, err := url.Parse(acsURL); err == nil { sp.AcsURL = *u }
    }
