...
func postgresSetup(t *testing.T) ssotest.Factory {
	t.Helper()
	...
	core := pgstore.New(db)
	require.NoError(t, core.Migrate(ctx, sso.PostgresMigrations), "run migrations")

	plugin := sso.NewPostgresStore(db)
	return func(t *testing.T) ssotest.Fixture {
		t.Helper()
...
func mongoSetup(t *testing.T) ssotest.Factory {
	t.Helper()
