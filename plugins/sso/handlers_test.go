...
func setupTestAppSSO(t *testing.T) (*bun.DB, *http.ServeMux) {
	...
	if _, err := db.NewCreateTable().Model((*schema.SSOProvider)(nil)).IfNotExists().Exec(ctx); err != nil {
		t.Fatalf("create sso_providers: %v", err)
	}
...
func TestSSO_RegisterAndCallbacks(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("oidc callback error: %v", err)
	}
	if resp2.StatusCode != 400 {
		t.Fatalf("expected 400 oidc callback (fake credentials), got %d", resp2.StatusCode)
	}
	...
	if err != nil {
		t.Fatalf("register provider saml error: %v", err)
	}
	if resp3.StatusCode != 200 {
		t.Fatalf("expected 200 register saml, got %d", resp3.StatusCode)
	}

	// SAML metadata
	resp4, err := http.Get(srv.URL + "/api/auth/sso/saml2/sp/metadata")
	if err != nil {
		t.Fatalf("saml md error: %v", err)
	}
	if resp4.StatusCode != 200 {
		t.Fatalf("expected 200 metadata, got %d", resp4.StatusCode)
	}
	...
	if err != nil {
		t.Fatalf("saml login error: %v", err)
	}
	if resp5.StatusCode != 200 {
		t.Fatalf("expected 200 saml login, got %d", resp5.StatusCode)
	}

	var loginResp map[string]any
	_ = json.NewDecoder(resp5.Body).Decode(&loginResp)
	if loginResp["redirect_url"] == "" {
		t.Fatalf("expected redirect_url in login response")
	}
	if loginResp["provider_id"] != "okta-saml" {
		t.Fatalf("expected provider_id okta-saml")
	}
	...
	if err != nil {
		t.Fatalf("saml callback error: %v", err)
	}
	if resp6.StatusCode != 200 {
		t.Fatalf("expected 200 saml callback, got %d", resp6.StatusCode)
	}
}
