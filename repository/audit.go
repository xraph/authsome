...
func (r *AuditRepository) List(ctx context.Context, limit, offset int) ([]*core.Event, error) {
    ...
    for i := range rows { out = append(out, r.fromSchema(&rows[i])) }
    return out, nil
}
...
func (r *AuditRepository) Search(ctx context.Context, params core.ListParams) ([]*core.Event, error) {
    ...
    if params.UserID != nil { q = q.Where("user_id = ?", params.UserID.String()) }
    if params.Action != "" { q = q.Where("action = ?", params.Action) }
    if params.Since != nil { q = q.Where("created_at >= ?", params.Since) }
    if params.Until != nil { q = q.Where("created_at <= ?", params.Until) }
    if err := q.Scan(ctx); err != nil { return nil, err }
    out := make([]*core.Event, 0, len(rows))
    for i := range rows { out = append(out, r.fromSchema(&rows[i])) }
    return out, nil
