...
func (r *IdentityVerificationRepository) GetVerificationStats(ctx context.Context, appID xid.ID, orgID xid.ID, from, to time.Time) (map[string]any, error) {
...
func (r *IdentityVerificationRepository) GetProviderStats(ctx context.Context, appID xid.ID, provider string, from, to time.Time) (map[string]any, error) {
	stats := map[string]any{
		"provider": provider,
	}
