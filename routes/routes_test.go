...
func (m *mockAuditRepo) SearchCount(ctx context.Context, params audit.ListParams) (int, error) { return 0, nil }
...
func (m *mockWebhookRepo) FindByID(ctx context.Context, id xid.ID) (*webhook.Webhook, error) { return nil, nil }
...
func (m *mockWebhookRepo) UpdateFailureCount(ctx context.Context, id xid.ID, count int) error { return nil }
func (m *mockWebhookRepo) UpdateLastDelivery(ctx context.Context, id xid.ID, timestamp time.Time) error { return nil }
func (m *mockWebhookRepo) CreateEvent(ctx context.Context, event *webhook.Event) error { return nil }
func (m *mockWebhookRepo) FindEventByID(ctx context.Context, id xid.ID) (*webhook.Event, error) { return nil, nil }
...
func (m *mockWebhookRepo) CreateDelivery(ctx context.Context, delivery *webhook.Delivery) error { return nil }
func (m *mockWebhookRepo) FindDeliveryByID(ctx context.Context, id xid.ID) (*webhook.Delivery, error) { return nil, nil }
...
func (m *mockWebhookRepo) UpdateDelivery(ctx context.Context, delivery *webhook.Delivery) error { return nil }
...
func TestOrganizationPolicyRoutes_CRUD(t *testing.T) {
    ...
    if err != nil { t.Fatalf("create policy request error: %v", err) }
    if resp.StatusCode != 201 { t.Fatalf("expected 201 create, got %d", resp.StatusCode) }

    // List
    resp2, err := http.Get(srv.URL+"/api/orgs/orgs/policies")
    if err != nil { t.Fatalf("list policies request error: %v", err) }
    if resp2.StatusCode != 200 {
    ...
    if err := json.NewDecoder(resp2.Body).Decode(&env); err != nil { t.Fatalf("decode list: %v", err) }
    if len(env.Data) == 0 { t.Fatalf("expected at least 1 policy after create") }
    ...
    if err != nil { t.Fatalf("update policy request error: %v", err) }
    if resp3.StatusCode != 200 {
        var e map[string]string
    ...
    if err != nil { t.Fatalf("delete policy request error: %v", err) }
    if resp4.StatusCode != 200 {
        var e map[string]string
    ...
    if err != nil { t.Fatalf("list after delete request error: %v", err) }
    if resp5.StatusCode != 200 { t.Fatalf("expected 200 list after delete, got %d", resp5.StatusCode) }
    var env2 PaginatedPolicies
    if err := json.NewDecoder(resp5.Body).Decode(&env2); err != nil { t.Fatalf("decode list2: %v", err) }
    if len(env2.Data) != 0 { t.Fatalf("expected 0 policies after delete, got %d", len(env2.Data)) }
}
...
func TestOrganizationPolicyRoutes_NegativeCases(t *testing.T) {
    ...
    if err != nil { t.Fatalf("bad create request error: %v", err) }
    if resp0.StatusCode != 400 { t.Fatalf("expected 400 create invalid, got %d", resp0.StatusCode) }
    ...
    if err != nil { t.Fatalf("create policy request error: %v", err) }
    if resp1.StatusCode != 201 { t.Fatalf("expected 201 create, got %d", resp1.StatusCode) }

    // List to get ID
    respList, err := http.Get(srv.URL+"/api/orgs/orgs/policies")
    if err != nil { t.Fatalf("list policies request error: %v", err) }
    if respList.StatusCode != 200 { t.Fatalf("expected 200 list, got %d", respList.StatusCode) }
    var envN PaginatedPolicies
    if err := json.NewDecoder(respList.Body).Decode(&envN); err != nil { t.Fatalf("decode list: %v", err) }
    if len(envN.Data) == 0 { t.Fatalf("expected at least 1 policy after create") }
    ...
    if err != nil { t.Fatalf("update no id request error: %v", err) }
    if resp2.StatusCode != 400 { t.Fatalf("expected 400 update missing id, got %d", resp2.StatusCode) }
    ...
    if err != nil { t.Fatalf("update invalid expr request error: %v", err) }
    if resp3.StatusCode != 400 { t.Fatalf("expected 400 update invalid expression, got %d", resp3.StatusCode) }
    ...
    if err != nil { t.Fatalf("delete missing id request error: %v", err) }
    if resp4.StatusCode != 400 { t.Fatalf("expected 400 delete missing id, got %d", resp4.StatusCode) }
}
...
func TestOrganizationPolicyRoutes_RBACEnforcement(t *testing.T) {
    ...
    if err := roleRepo.Create(ctx, ownerRole); err != nil { t.Fatalf("create owner role: %v", err) }
    adminRole := &schema.Role{ Name: "admin", Description: "Admin role" }
    if err := roleRepo.Create(ctx, adminRole); err != nil { t.Fatalf("create admin role: %v", err) }
    ...
    orgID := xid.New()
    if err := userRoleRepo.Assign(ctx, ownerUserID, ownerRole.ID, orgID); err != nil { t.Fatalf("assign owner role: %v", err) }
    if err := userRoleRepo.Assign(ctx, adminUserID, adminRole.ID, orgID); err != nil { t.Fatalf("assign admin role: %v", err) }

    // Verify role retrieval for owner before making requests
    ownerRoles, err := userRoleRepo.ListRolesForUser(ctx, ownerUserID, nil)
    if err != nil { t.Fatalf("list roles for owner error: %v", err) }
    if len(ownerRoles) == 0 { t.Fatalf("owner should have at least one role; got 0") }
    // Verify role retrieval for admin
    adminRoles, err := userRoleRepo.ListRolesForUser(ctx, adminUserID, nil)
    if err != nil { t.Fatalf("list roles for admin error: %v", err) }
    if len(adminRoles) == 0 { t.Fatalf("admin should have at least one role; got 0") }
    ...
    if err != nil { t.Fatalf("owner create policy request error: %v", err) }
    ...
    if err != nil { t.Fatalf("owner list policies request error: %v", err) }
    if respList.StatusCode != 200 { t.Fatalf("expected 200 owner list, got %d", respList.StatusCode) }
    var envR PaginatedPolicies
    if err := json.NewDecoder(respList.Body).Decode(&envR); err != nil { t.Fatalf("decode owner list: %v", err) }
    if len(envR.Data) == 0 { t.Fatalf("expected policies present for owner") }
    ...
    if err != nil { t.Fatalf("owner update policy request error: %v", err) }
    ...
    if err != nil { t.Fatalf("owner delete policy request error: %v", err) }
    ...
    if err != nil { t.Fatalf("admin create policy request error: %v", err) }
    if respCreate2.StatusCode != 403 { t.Fatalf("expected 403 admin create, got %d", respCreate2.StatusCode) }
    ...
    if err != nil { t.Fatalf("admin list policies request error: %v", err) }
    if respList2.StatusCode != 200 { t.Fatalf("expected 200 admin list, got %d", respList2.StatusCode) }
    ...
    if err != nil { t.Fatalf("admin update policy request error: %v", err) }
    if respUpd2.StatusCode != 403 { t.Fatalf("expected 403 admin update, got %d", respUpd2.StatusCode) }
    ...
    if err != nil { t.Fatalf("admin delete policy request error: %v", err) }
    if respDel2.StatusCode != 403 { t.Fatalf("expected 403 admin delete, got %d", respDel2.StatusCode) }
}
