...
func TestEvaluator_Evaluate(t *testing.T) {
    ...
    if !ok { t.Fatalf("expected allowed for matching action and resource") }
    ...
    if !ok { t.Fatalf("expected allowed when condition matches") }
    ok = e.Evaluate(p, &Context{Subject: "user", Action: "read", Resource: "project:123", Vars: map[string]string{"role": "user"}})
    if ok { t.Fatalf("expected deny when condition fails") }
}