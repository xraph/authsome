...
func init() {
	Migrations.MustRegister(func(ctx context.Context, db *bun.DB) error {
		...
		fmt.Println("[Migration] Adding 'app_id' and 'organization_id' to 'permission_policies'...")
		...
		`)
		if err != nil {
			return fmt.Errorf("failed to add app_id and organization_id to permission_policies: %w", err)
		}
		fmt.Println("[Migration] 'app_id' and 'organization_id' added to 'permission_policies'.")
		...
		fmt.Println("[Migration] Adding 'app_id' and 'organization_id' to 'permission_namespaces'...")
		_, err = db.ExecContext(ctx, `
		...
		`)
		if err != nil {
			return fmt.Errorf("failed to add app_id and organization_id to permission_namespaces: %w", err)
		}
		fmt.Println("[Migration] 'app_id' and 'organization_id' added to 'permission_namespaces'.")
		...
		fmt.Println("[Migration] Adding 'app_id' and 'organization_id' to 'permission_audit_events'...")
		_, err = db.ExecContext(ctx, `
		...
		`)
		if err != nil {
			return fmt.Errorf("failed to add app_id and organization_id to permission_audit_events: %w", err)
		}
		fmt.Println("[Migration] 'app_id' and 'organization_id' added to 'permission_audit_events'.")
		...
		_, err = db.ExecContext(ctx, `
			ALTER TABLE permission_policies
			ADD CONSTRAINT fk_permission_policies_organization_id FOREIGN KEY (organization_id) REFERENCES organizations (id) ON DELETE CASCADE;
		`)
		if err != nil {
			return fmt.Errorf("failed to add fk_permission_policies_organization_id: %w", err)
		}
		...
		_, err = db.ExecContext(ctx, `
			ALTER TABLE permission_namespaces
			ADD CONSTRAINT fk_permission_namespaces_organization_id FOREIGN KEY (organization_id) REFERENCES organizations (id) ON DELETE CASCADE;
		`)
		if err != nil {
			return fmt.Errorf("failed to add fk_permission_namespaces_organization_id: %w", err)
		}
		...
		_, err = db.ExecContext(ctx, `
			ALTER TABLE permission_audit_events
			ADD CONSTRAINT fk_permission_audit_events_organization_id FOREIGN KEY (organization_id) REFERENCES organizations (id) ON DELETE CASCADE;
		`)
		if err != nil {
			return fmt.Errorf("failed to add fk_permission_audit_events_organization_id: %w", err)
		}
		...
		_, err = db.ExecContext(ctx, `
			...
			CREATE INDEX idx_permission_policies_app_org ON permission_policies (app_id, organization_id);
		`)
		if err != nil {
		...
		_, err = db.ExecContext(ctx, `
			...
			CREATE INDEX idx_permission_namespaces_organization_id ON permission_namespaces (organization_id);
		`)
		if err != nil {
		...
		_, err = db.ExecContext(ctx, `
			...
			CREATE INDEX idx_permission_audit_events_organization_id ON permission_audit_events (organization_id);
		`)
		if err != nil {
		...
		db.ExecContext(ctx, `ALTER TABLE permission_policies DROP CONSTRAINT IF EXISTS fk_permission_policies_app_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_policies DROP CONSTRAINT IF EXISTS fk_permission_policies_organization_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_namespaces DROP CONSTRAINT IF EXISTS fk_permission_namespaces_app_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_namespaces DROP CONSTRAINT IF EXISTS fk_permission_namespaces_organization_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_audit_events DROP CONSTRAINT IF EXISTS fk_permission_audit_events_app_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_audit_events DROP CONSTRAINT IF EXISTS fk_permission_audit_events_organization_id;`)

		db.ExecContext(ctx, `DROP INDEX IF EXISTS idx_permission_policies_app_id;`)
		db.ExecContext(ctx, `DROP INDEX IF EXISTS idx_permission_policies_organization_id;`)
		db.ExecContext(ctx, `DROP INDEX IF EXISTS idx_permission_policies_app_org;`)
		db.ExecContext(ctx, `DROP INDEX IF EXISTS idx_permission_namespaces_app_id;`)
		db.ExecContext(ctx, `DROP INDEX IF EXISTS idx_permission_namespaces_organization_id;`)
		db.ExecContext(ctx, `DROP INDEX IF EXISTS idx_permission_audit_events_app_id;`)
		db.ExecContext(ctx, `DROP INDEX IF EXISTS idx_permission_audit_events_organization_id;`)
		...
		db.ExecContext(ctx, `ALTER TABLE permission_policies ADD COLUMN org_id VARCHAR(20) NULL;`)
		db.ExecContext(ctx, `UPDATE permission_policies SET org_id = app_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_policies DROP COLUMN app_id, DROP COLUMN organization_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_policies ALTER COLUMN org_id SET NOT NULL;`)
		...
		db.ExecContext(ctx, `ALTER TABLE permission_namespaces ADD COLUMN org_id VARCHAR(20) NULL;`)
		db.ExecContext(ctx, `UPDATE permission_namespaces SET org_id = app_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_namespaces DROP COLUMN app_id, DROP COLUMN organization_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_namespaces ALTER COLUMN org_id SET NOT NULL;`)
		...
		db.ExecContext(ctx, `ALTER TABLE permission_audit_events ADD COLUMN org_id VARCHAR(20) NULL;`)
		db.ExecContext(ctx, `UPDATE permission_audit_events SET org_id = app_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_audit_events DROP COLUMN app_id, DROP COLUMN organization_id;`)
		db.ExecContext(ctx, `ALTER TABLE permission_audit_events ALTER COLUMN org_id SET NOT NULL;`)
