...
func init() {
	Migrations.MustRegister(func(ctx context.Context, db *bun.DB) error {
		...
		// Adds app_id and organization_id to phone_verifications table
		...
		// Create index on (app_id, organization_id, phone) for efficient lookups
		_, err = db.ExecContext(ctx, `
			CREATE INDEX IF NOT EXISTS idx_phone_verifications_app_org_phone 
			ON phone_verifications (app_id, organization_id, phone)
		`)
		if err != nil {
