...
Several designs dated 2026-08-24 add columns to the same two tables, and two
migrations sharing a version inside one group fail at startup. This plan claims:

| Group | Version | Migration | Table |
|---|---|---|---|
| `authsome` | `20260824000040` | `add_session_dpop_jkt` | `authsome_sessions` |
| `authsome-oauth2` | `20260824000041` | `add_oauth2_client_dpop_mode` | `authsome_oauth2_clients` |

Expect company on both tables. The agentauth delegation plan and the RFC 8707
resource indicators design each add a column to `authsome_sessions`, and the
dynamic client registration plan (which already moved to `20260824000030` for
this reason) plus the same 8707 design add columns to
`authsome_oauth2_clients`.

If you hit a clash, the resolution is always the union of the columns and a
fresh unused version number, never dropping somebody else's column. Before
adding either migration, grep the group for the version you intend to use:

```bash
grep -rn "2026082400" store/postgres/migrations.go plugins/oauth2provider/migrations.go
```
...
In `store/sqlite/migrations.go`, add the equivalent using the same version string `20260824000040` and name `add_session_dpop_jkt`:
...
In `plugins/oauth2provider/migrations.go`, add to both `PostgresMigrations` and `SqliteMigrations`, name `add_oauth2_client_dpop_mode`, version `20260824000041`:
