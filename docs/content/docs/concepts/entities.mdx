...
| `Actors` | The chain of principals acting on the subject's behalf. Call `ImpersonatedBy()` to check whether this is an admin impersonation session; it returns the admin's non-nil UserID when it is. |
