...
- **`Actors`** carries the admin when one creates a session on behalf of another user; call **`ImpersonatedBy()`** to read it back (see [Impersonation](/docs/users-sessions/impersonation)).
