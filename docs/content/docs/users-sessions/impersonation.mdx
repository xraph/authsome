...
When an admin impersonates a user, the engine creates a new session for the target user and records the admin as an actor on that session's actor chain. The resulting session token behaves exactly like a normal session for the target user, except:

1. The session's actor chain carries the admin, and `Session.ImpersonatedBy()` returns the admin's user ID
...
// ImpersonatedBy returns the admin acting as this session's user, or the
// zero ID when nobody is. Derived from Actors rather than stored.
...
You can identify impersonation sessions by calling the `ImpersonatedBy()` method:
