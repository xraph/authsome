...
| `@authsome/ui-react` | — | `AuthProvider`, `useAuth`, `useUser`, `useOrganizations`, `useSessionToken`, headless `SignInForm`, `SignUpForm`, `MFAChallengeForm`, `AuthGuard` |
| `@authsome/ui-components` | — | 40+ styled components built with Radix UI primitives and Tailwind CSS |
| `@authsome/ui-nextjs` | — | `getServerSession()` for RSC, `createCookieStorage()`, `createAuthMiddleware()` for Edge, pre-built Next.js auth pages |
...
<Cards>
  ...
  <Card title="Next.js Integration" href="/docs/ui/nextjs" description="Server sessions, Edge middleware, pre-built pages." />
